...
What it produces (under --out-dir, default ``knowledge/_meta/graph/``):
  ...
  * ``graph.html``    — a self-contained interactive viz (open in any browser;
    ...
    Gephi/yEd.
  * ``GRAPH_REPORT.md`` — a deterministic human-readable summary (overview,
    god_nodes, bridges, labelled communities, asserted relations, suggested
    questions). The static sibling of ``graph.html``; no timestamp, so an
    unchanged wiki reproduces it byte-for-byte. Also available on stdout via the
    ``report`` sub-command.
...
Derived views
  ...
  * ``communities`` — thematic clusters (greedy modularity), each carrying a
    deterministic LLM-free ``label`` = the title of its most-connected member.
...
def _community_label(members, degree, title_of) -> str:
    """Deterministic, LLM-free community label: the title of the most-connected
    member (the cluster's local hub), tie-broken by slug. Falls back to the slug
    when a node has no title. Source titles are trimmed to their "Author Year"
    head so the label stays compact enough for the viz and the report.
    """
    rep = min(members, key=lambda m: (-degree.get(m, 0.0), m))
    title = str(title_of.get(rep) or rep).strip()
    head = re.split(r"\s[—–-]\s", title, maxsplit=1)[0].strip() or title
    return head if len(head) <= 40 else head[:39].rstrip() + "…"
...
def compute_communities(nodes, edges):
    ...
    title_of = {n["id"]: n.get("title", n["id"]) for n in nodes}
    for cid, members in enumerate(ordered):
        ...
        communities.append({"community": cid, "size": len(members),
                            "label": _community_label(members, deg, title_of),
...
def build_report_md(nodes, edges, god_nodes, bridges, communities, stats) -> str:
    """Deterministic, human-readable summary of the graph — the static sibling of
    graph.html. No timestamp and no randomness, so re-running on an unchanged
    wiki yields a byte-identical file (clean git diffs). Everything here is read
    straight off the computed graph; nothing is inferred beyond what the edges
    already assert. Page ids are emitted as ``[[wikilinks]]`` so the report is
    navigable in Foam/Obsidian.
    """
    title_of = {n["id"]: n.get("title", n["id"]) for n in nodes}

    def name(slug):
        return title_of.get(slug) or slug
    ...
    inf_pct = round(100 * ia / rt) if rt else 0
    dangling = stats.get("dangling", 0)

    L = ["# Knowledge-graph report", ""]
    L.append("_Generated by `scripts/wiki-to-graph.py`. Deterministic — every claim "
             "below traces back to `graph.json`; re-running on an unchanged wiki "
             "reproduces this file exactly._")
    L.append("")

    # Overview -------------------------------------------------------------
    L += ["## Overview", ""]
    L.append(f"- **{len(nodes)} pages**, **{len(edges)} links**, "
             f"**{len(communities)} communities**")
    if by_type:
        L.append("- By type: " + ", ".join(f"{v} {k}" for k, v in sorted(by_type.items())))
    L.append(f"- Typed relations: {rt} · **inference-rate {inf_pct}%** "
             f"({ia} of {rt} model-asserted rather than grounded)")
    if dangling:
        L.append(f"- ⚠️ **{dangling} dangling link(s)** to pages that don't exist — "
                 f"run `lint-wiki.py`, then `ingest-source` for the missing pages")
    else:
        L.append("- No dangling links — every reference resolves")
    L.append("")

    # God nodes ------------------------------------------------------------
    L += ["## Most-connected pages (god nodes)", ""]
    L.append("_Where the wiki converges. High degree marks a hub, not automatically "
             "importance — judge each._")
    L.append("")
    if god_nodes:
        L += ["| Degree | Type | Page |", "| ---: | --- | --- |"]
        for g in god_nodes:
            L.append(f"| {g['degree']} | {g['type']} | [[{g['id']}]] — {name(g['id'])} |")
    else:
        L.append("_None — the graph has no links yet._")
    L.append("")

    # Bridges --------------------------------------------------------------
    L += ["## Bridges (load-bearing connectors)", ""]
    L.append("_Entities joining source clusters that share no other entity. Remove one "
             "and parts of the literature fall apart — often the most analytically "
             "interesting nodes._")
    L.append("")
    if bridges:
        for b in bridges:
            srcs = ", ".join(f"[[{s}]]" for s in b["sources"])
            L.append(f"- **[[{b['id']}]]** — joins {b['connects']} clusters across "
                     f"{len(b['sources'])} sources: {srcs}")
    else:
        L.append("_None — no entity bridges otherwise-unconnected sources._")
    L.append("")

    # Communities ----------------------------------------------------------
    L += ["## Communities (thematic clusters)", ""]
    L.append("_Auto-detected clusters (greedy modularity). Each label is the title of "
             "the cluster's most-connected page, not an LLM summary._")
    L.append("")
    sized = [c for c in communities if c["size"] >= 2]
    if sized:
        for c in sized:
            bt = ", ".join(f"{v} {k}" for k, v in sorted(c["by_type"].items()))
            members = ", ".join(f"[[{m}]]" for m in c["members"][:8])
            more = f" … (+{c['size'] - 8})" if c["size"] > 8 else ""
            L.append(f"- **{c.get('label', '')}** — {c['size']} pages ({bt}): {members}{more}")
    else:
        L.append("_No multi-page clusters yet._")
    L.append("")

    # Asserted (typed) relations ------------------------------------------
    typed = [e for e in edges if e["relation_type"] != "wikilink"]
    if typed:
        L += ["## Asserted relations", ""]
        L.append("_Typed `relations:` edges — the semantically explicit links "
                 "(contradictions, build-on chains, …). `(inferred)` / `(ambiguous)` "
                 "marks an edge not directly grounded in a source._")
        L.append("")
        buckets: dict[str, list] = {}
        for e in typed:
            buckets.setdefault(e["relation_type"], []).append(e)
        for rtype in sorted(buckets):
            L.append(f"### {rtype}")
            for e in buckets[rtype]:
                conf = "" if e.get("confidence") == "extracted" else f" _({e.get('confidence')})_"
                why = f" — {e['because']}" if e.get("because") else ""
                L.append(f"- [[{e['source']}]] → [[{e['target']}]]{conf}{why}")
            L.append("")

    # Suggested questions --------------------------------------------------
    L += ["## Suggested questions", ""]
    L.append("_Starting points generated from the structure above — each names a real "
             "page so you can follow it up in the wiki._")
    L.append("")
    qs = []
    if god_nodes:
        g = god_nodes[0]
        qs.append(f"What makes **{name(g['id'])}** so central ({g['degree']} links) — "
                  f"analytic weight, or just a generic hub?")
    for b in bridges[:3]:
        qs.append(f"How do the sources connected only through **{name(b['id'])}** "
                  f"actually relate — is the bridge substantive or incidental?")
    contradictions = [e for e in typed if e["relation_type"] == "contradicts"]
    for e in contradictions[:3]:
        qs.append(f"How is the contradiction between **{name(e['source'])}** and "
                  f"**{name(e['target'])}** resolved in your argument?")
    if inf_pct >= 40:
        qs.append(f"The inference-rate is {inf_pct}% — which asserted relations still "
                  f"need grounding in a source before they carry weight?")
    if dangling:
        qs.append(f"{dangling} link(s) point to pages that don't exist yet — which of "
                  f"those should be ingested next?")
    if not qs:
        qs.append("The graph is still sparse — ingest a few more sources, then re-run "
                  "to surface hubs and bridges.")
    L += [f"- {q}" for q in qs]
    L.append("")
...
def write_report(out_dir: Path, nodes, edges, god_nodes, bridges, communities, stats) -> Path:
    path = out_dir / "GRAPH_REPORT.md"
    md = build_report_md(nodes, edges, god_nodes, bridges, communities, stats)
    path.write_text(md + "\n", encoding="utf-8")
    return path
...
def write_html(out_dir: Path, nodes, edges, bridges, stats, vendor_path: Path,
               community_labels: dict | None = None) -> Path | None:
    ...
    def _clabel(c):
        name = community_labels.get(c)
        return f"{name} ({comm_size[c]})" if name else f"community {c} ({comm_size[c]})"
...
def run_query(args, nodes, edges, stats) -> int:
    ...
    if args.cmd == "communities":
        ...
        for c in comms:
            ...
            print(f"  #{c['community']}{label}  ({c['size']}: {bt})  {preview}")
        return 0

    if args.cmd == "report":
        god_nodes = compute_god_nodes(nodes, edges, args.top_n)
        bridges = compute_bridges(nodes, edges)
        _, communities = compute_communities(nodes, edges)
        print(build_report_md(nodes, edges, god_nodes, bridges, communities, stats))
        return 0
...
def main() -> int:
    ...
    sp = sub.add_parser("communities", parents=[common], help="Detected thematic clusters (greedy modularity, labelled)")
    ...
    sp = sub.add_parser("report", parents=[common],
                        help="Print GRAPH_REPORT.md to stdout (overview + suggested questions)")
    sp.add_argument("--top-n", type=int, default=DEFAULT_TOP_N)
    ...
    report_path = write_report(args.out_dir, nodes, edges, god_nodes, bridges, communities, stats)
    ...
    if not args.no_html:
        ...
        community_labels = {c["community"]: c["label"] for c in communities}
        html_path = write_html(args.out_dir, nodes, edges, bridges, stats, vendor, community_labels)
        if html_path:
